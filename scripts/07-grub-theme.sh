...
if [ -d "$SOURCE_BASE" ]; then
    ...
    for dir in "$SOURCE_BASE"/*; do
        ...
        # wuthering/ is a template: generate one theme per background image
        if [ "$THEME_BASENAME" = "wuthering" ]; then
            if [ -d "$dir/backgrounds" ] && [ -f "$dir/theme.txt" ]; then
                for bg in "$dir/backgrounds"/*.jpg; do
                    ...
                    if [ ! -d "$DEST_DIR/$BG_NAME" ]; then
                        log "Installing $BG_NAME to system (from wuthering)..."
                        ...
                        # Icons
                        exe cp -r "$dir/icons" "$DEST_DIR/$BG_NAME/"
                        # UI images (logo, info, select_*)
